...
class UpcommingComponent extends StatelessWidget {
  const UpcommingComponent({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<MovieBloc, MovieState>(
      buildWhen: (previous, current) =>
          previous.upcomminMoviesRequestState !=
          current.upcomminMoviesRequestState,
      builder: (context, state) {
        switch (state.upcomminMoviesRequestState) {
          case RequestState.loading:
            return const ShimmerLoading();
          case RequestState.loaded:
            if (state.upcommingMovies.isEmpty) {
              ...
              return FadeIn(
                ...
                child: Center(
                  child: Wrap(
                    runSpacing: 10.h,
                    children:
                        List.generate(state.upcommingMovies.length, (index) {
                      final movie = state.upcommingMovies[index];
          ...
          case RequestState.error:
            return ErrorComponent(message: state.upcomminMoviesMessage);
        }
      },
    );
  }
